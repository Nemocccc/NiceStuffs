...
if not exist "%PKG_CLAUDE%\bin\claude.exe" (echo  [MISS] claude.exe                 & set MISS=1)
...
echo [4/5] Installing Claude Code v%CLI_VER%...
if not exist "%CLI_DIR%" mkdir "%CLI_DIR%"
xcopy /E /I /Y "%PKG_CLAUDE%\*" "%CLI_DIR%\" >nul 2>&1
if %errorlevel% neq 0 (echo [FAIL] xcopy Claude & pause & exit /b 1)
if not exist "%CLI_DIR%\bin\claude.exe" (echo [FAIL] claude.exe missing after copy & pause & exit /b 1)

> "%NVM_DIR%\claude.cmd" (
    echo @echo off
    echo set "PATH=%%USERPROFILE%%\.nvm\nodejs;%%PATH%%"
    echo "%%USERPROFILE%%\claude-code\bin\claude.exe" %%*
)
