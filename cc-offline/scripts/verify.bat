...
setlocal
echo ---------------------------------------------------------------
echo  Claude Code Post-Install Verification
echo ---------------------------------------------------------------
echo.
set ERR=0
...
if %errorlevel% equ 0 (
    for /f "tokens=*" %%a in ('claude --version 2^>nul') do echo    %%a
) else (
    echo    FAIL - claude not found
    echo    (check %%USERPROFILE%%\.nvm\claude.cmd)
    set ERR=1
)

echo [4/5] ANTHROPIC_BASE_URL...
reg query "HKCU\Environment" /v ANTHROPIC_BASE_URL >nul 2>&1
if %errorlevel% equ 0 (
    for /f "skip=2 tokens=3*" %%a in ('reg query "HKCU\Environment" /v ANTHROPIC_BASE_URL') do echo    %%a
) else (
    echo    NOT SET
    set ERR=1
)
