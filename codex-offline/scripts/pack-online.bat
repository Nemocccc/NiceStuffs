...
echo ---------------------------------------------------------------
echo  Codex CLI Offline Packer
echo  Downloads nvm, Node.js, and Codex CLI for offline install.
...
if exist "codex-offline\node_modules\@openai\codex\bin\claude.exe" (
    echo [SKIP] Codex CLI already downloaded
) else (
    echo [3/3] Downloading Codex CLI (requires npm, may take several minutes)...
    set PATH=%CD%\node\%NODE_ZIP%;%PATH%
    if not exist "codex-offline" mkdir codex-offline
    cd codex-offline
    ...
    call npm pack @openai/codex >nul 2>&1
    cd ..
    if exist "codex-offline\node_modules\@openai\codex\bin\claude.exe" (
        echo [OK]
    ) else (
...
echo.
echo  Total size:
dir /s /-c nvm node codex-offline 2>nul | find "File(s)"
echo.
